...
void Athi_Checkbox::update() {
  ...
  if (last_state == PRESSED && get_status() == HOVER)
    this_state = TOGGLE;
  switch (this_state) {
  case HOVER:
...
bool Athi_Checkbox::hover_over() {
  ...
  if (mouse_x > box.pos.x && mouse_x < box.pos.x + box.width &&
