...
void collisionResolve(u32 a_id, u32 b_id) {
  ...
  if (d < 0.0) {
    ...
    athi_circle_manager->circle_buffer[a_id]->vel =
        (scal_norm_1_vec + scal_norm_1_after_vec) * 0.95f;
    athi_circle_manager->circle_buffer[b_id]->vel =
        (scal_norm_2_vec + scal_norm_2_after_vec) * 0.95f;
  }
}
...
void Athi_Circle_Manager::init() {
  ...
  const u32 vs =
      createShader("../Resources/athi_circle_shader.vs", GL_VERTEX_SHADER);
  const u32 fs =
      createShader("../Resources/athi_circle_shader.fs", GL_FRAGMENT_SHADER);
...
void Athi_Circle_Manager::draw() {
  ...
  glDrawArraysInstanced(GL_LINE_LOOP, 0, CIRCLE_NUM_VERTICES,
                        (s32)circle_buffer.size());
}
  ...
  if (circle_collision) {
    ...
    if (quadtree_active || voxelgrid_active)
      ...
      for (auto &thread : threads) {
        thread = std::thread(&Athi_Circle_Manager::collision_logNxN, this,
                             parts * i, parts * (1 + i));
        ++i;
      }
...
void Athi_Circle_Manager::collision_logNxN(size_t begin, size_t end) {
  for (size_t i = begin; i < end; ++i)
    for (size_t j = 1 + i; j < circle_buffer.size(); ++j)
      if (collisionDetection(i, j)) collisionResolve(i, j);
}

void Athi_Circle_Manager::collision_quadtree(
    const std::vector<std::vector<u32> > &cont, size_t begin, size_t end) {
