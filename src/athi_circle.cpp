...
void Athi_Circle_Manager::collision_quadtree(
    const std::vector<std::vector<int>> &cont, size_t begin, size_t end) {
...
void add_circle_multiple(Athi_Circle &circle, int num) {
  const float radi = circle.radius;
...
int get_num_circles() { return (int)athi_circle_manager->circle_buffer.size(); }
