...
Athi_Text_Manager::~Athi_Text_Manager() { glDeleteVertexArrays(1, &VAO); }
...
void Athi_Text_Manager::init() {
  ...
  const u32 fs =
      createShader("../Resources/text_shader.fs", GL_FRAGMENT_SHADER);
  ...
  uniform[TEXTCOORD_INDEX] =
      glGetUniformLocation(shader_program, "textCoord_index");
...
void draw_all_text() { athi_text_manager.draw(); }
