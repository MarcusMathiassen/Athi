...
private:
  ...
  std::unique_ptr<Quadtree_v2> sw;
  std::unique_ptr<Quadtree_v2> se;
  std::unique_ptr<Quadtree_v2> nw;
  std::unique_ptr<Quadtree_v2> ne;
  ...
  void split() noexcept {
    ...
    sw = std::make_unique<Quadtree_v2>(level + 1, SW);
    se = std::make_unique<Quadtree_v2>(level + 1, SE);
    nw = std::make_unique<Quadtree_v2>(level + 1, NW);
    ne = std::make_unique<Quadtree_v2>(level + 1, NE);
  }
  ...
  bool contains(std::int32_t id) const noexcept { return bounds.contains(pos[id], radii[id]); }

public:
  Quadtree_v2(std::int32_t level, const Rect &bounds) noexcept : bounds(bounds), level(level) {}
  Quadtree_v2(const glm::vec2 &min, const glm::vec2 &max) noexcept {
    bounds.color = pastel_gray;
    bounds.min_pos = min;
    bounds.max_pos = max;
  }
  void input(std::int32_t count, glm::vec2 *pos_, float* radii_) noexcept {
    pos = pos_;
    radii = radii_;
  ...
  void get(std::vector<std::vector<std::int32_t>> &cont) const noexcept {
    if (sw) {
      sw->get(cont);
      se->get(cont);
      nw->get(cont);
